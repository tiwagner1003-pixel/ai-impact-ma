...
```bash
uv run .Codex/skills/argdown/scripts/validate.py <file.argdown>
```
...
```bash
# PNG (default, 150 DPI)
uv run .Codex/skills/argdown/scripts/render.py <file.argdown>

# SVG
uv run .Codex/skills/argdown/scripts/render.py <file.argdown> --format svg

# All formats (DOT + SVG + PNG)
uv run .Codex/skills/argdown/scripts/render.py <file.argdown> --format all

# JSON (structured data, no Graphviz needed)
uv run .Codex/skills/argdown/scripts/render.py <file.argdown> --format json

# High-DPI PNG
uv run .Codex/skills/argdown/scripts/render.py <file.argdown> --dpi 300

# Dark theme
uv run .Codex/skills/argdown/scripts/render.py <file.argdown> --theme dark

# With legend
uv run .Codex/skills/argdown/scripts/render.py <file.argdown> --legend

# Alternative layout engine
uv run .Codex/skills/argdown/scripts/render.py <file.argdown> --engine fdp

# Debug: dump parsed model
uv run .Codex/skills/argdown/scripts/render.py <file.argdown> --dump-model
```
...
```bash
uv run .Codex/skills/argdown/scripts/validate.py <file.argdown>
uv run .Codex/skills/argdown/scripts/validate.py <file.argdown> --strict   # warnings = errors
uv run .Codex/skills/argdown/scripts/validate.py *.argdown                 # batch validate
```

For quick pattern-based checks (no dependencies):

```bash
bash .Codex/skills/argdown/scripts/validate-argdown.sh <file.argdown>
```
